...
import numpy as np
import pytest
from experiments import e136_transport_consistency as e
...
def test_actual_small_fits_exclude_outer_fold_and_lock_before_metrics(tmp_path,monkeypatch):
    root=tmp_path/'e136';root.mkdir();base=tmp_path/'e131';base.mkdir();evidence=tmp_path/'evidence';evidence.mkdir()
    ...
    fits=[];actual_fit=e.transport_consistency.fit_head
    def fit(x,*args,**kwargs):
        fits.append(set(x[:,0].astype(int).tolist()));return actual_fit(x,*args,**kwargs)
    monkeypatch.setattr(e.transport_consistency,'fit_head',fit)

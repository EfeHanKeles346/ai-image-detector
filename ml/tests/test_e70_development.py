...
import numpy as np
import pytest
from experiments import e70_development as d
from experiments.e67_features import blur_crops


def test_dev_blur_exactly_matches_frozen_train_transform():
    crops=np.random.default_rng(67).integers(0,256,(3,224,224,3),dtype=np.uint8)
    cached=np.stack([crops,crops,crops])
    assert np.array_equal(np.stack(d.blur_view(crops)),blur_crops(cached)[0])
...
def test_failed_train_guard_cannot_create_dev_contract(tmp_path,monkeypatch):
    ...
    (tmp_path/'e70_fit.json').write_text(payload)

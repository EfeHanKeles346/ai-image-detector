"""Convex TRAIN correction with explicit preservation of reference AI decisions."""
import numpy as np
...
def fit(x, baseline_logits, labels, weights, check=lambda: None):
    ...
    constraints = [LinearConstraint(x[protected_ai], -slack, np.inf),
                   LinearConstraint(x[protected_real], -np.inf, 0)]
    trace = []
    def callback(w):
        check()
    ...
    violation = max(0., float(np.max(-slack - shift[protected_ai])),
                    float(np.max(shift[protected_real])))

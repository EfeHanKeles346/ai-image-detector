"""One consumed E66 comparison from frozen encoder caches after full E103 TRAIN/runtime pass."""
...
if __name__=='__main__':
    def denied(*a,**kw):raise RuntimeError('E103 consumed DEV offline')
...
from experiments.e103_fit import validate as validate_fit,CONTRACT as FIT_CONTRACT,CANDIDATE,REPORT as FIT_REPORT,ROOT,EVIDENCE
...
PREVIOUS_SCORES=DATA_ROOT/'e102/dev_scores.json';PREVIOUS_REPORT=DATA_ROOT/'e102/dev_report.json'
...
def validate_predecessor(rows, cached, manifest):
    ...
    if any(any(a[k]!=b[k] for k in fields) for a,b in zip(rows,cached,strict=True)):
        raise ValueError('E102 predecessor and E83 cache identities/order differ')


def freeze():
    validate_fit();fit=read(FIT_REPORT)
    if digest(FIT_REPORT)!=digest(EVIDENCE/'e103_fit.json') or not fit['dev_scoring_permitted']:
        raise ValueError('E103 complete TRAIN/runtime pass required before DEV cache access')
    ...
    if digest(PREVIOUS_REPORT)!=digest(EVIDENCE/'e102_development.json') or digest(PREVIOUS_SCORES)!=previous['scores_sha256']:
        raise ValueError('immutable E102 predecessor report/scores required')
    if not all(v['zero_lost_E92_ai'] and v['zero_new_E92_real_errors'] for v in previous['checks'].values()):
        raise ValueError('E102 predecessor must retain E92 AI and correct REAL decisions')
    ...
    c={'state':'E103_single_consumed_E66_cache_comparison_registered','inputs':{str(p):digest(p) for p in paths},
       ...
       'features':'Exact complete E83 DEVELOPMENT DINO3072/CLIP1536/DEAR1640 arrays, ordered original/Q75 image '
                  ...
                  'Frozen E103 map in runtime batch8. Replay every E43 reference score<=1e-6 and both cuts unchanged.',
       'dev_status':'CONSUMED DEVELOPMENT already used by E70/E71/E83/E102. Neither fresh nor independent final.',
       'comparison':'All20 numeric gates; zero lost E43 or E102 AI and zero new E102 REAL errors in each source/condition. '
                    'non-increased pooled REAL FPR in both conditions. Also report all paired source transitions '
                    'against rejected E102 so losing its rescues cannot be hidden. Scores locked before metrics.',
       'limits':cache['limits']+' E83/E102 DEV labels/features remain excluded from TRAIN; cached comparison does not reset consumption.',
    ...
    write_once(CONTRACT,c);write_once(EVIDENCE/'e103_dev_contract.json',c|{'contract_sha256':digest(CONTRACT)})
    return {'state':c['state'],'views':c['views']}


def validate():
    validate_fit();validate_cache();c=read(CONTRACT)
    if digest(CONTRACT)!=read(EVIDENCE/'e103_dev_contract.json')['contract_sha256']:raise ValueError('E103 DEV contract changed')
    for p,s in c['inputs'].items():
        if digest(p)!=s:raise ValueError('E103 DEV input changed: '+p)
...
def score():
    c=validate()
    if SCORES.exists():raise FileExistsError('E103 DEV scores already locked')
    ...
    result={'state':'E103_DEV_scores_locked_before_metrics','contract_sha256':digest(CONTRACT),'rows':done,
            'prior_reference_max_error':max_error,'reference_decision_changes':0,'features_sha256':digest(FEATURES),
            'image_reads':0,'seconds':time.monotonic()-start,'previously_consumed_by':['E70','E71','E83','E86','E92','E102']}
    write_once(SCORES,result);write_once(EVIDENCE/'e103_dev_scores.json',{k:v for k,v in result.items() if k!='rows'}|
...
def report():
    c=validate()
    if digest(SCORES)!=read(EVIDENCE/'e103_dev_scores.json')['scores_sha256']:raise ValueError('locked E103 scores changed')
    ...
    for condition in CONDITIONS:
        ...
        reports[condition]={'old':old,'new':new,'transitions':change,'real_scenes':scenes,
                            'E102_predecessor':predecessor,'transitions_vs_E102':previous_changes}
        checks[condition]={'absolute_gates_passed':new['gate']['passed'],
            'zero_new_ai_misses':all(v['new_ai_misses']==0 for v in change.values()),
            'zero_lost_E102_ai':all(v['new_ai_misses']==0 for v in previous_changes.values()),
            'zero_new_E102_real_errors':all(v['new_real_false_ai']==0 for v in previous_changes.values()),
            'real_pooled_not_increased':new['binary_rates']['pooled_real_false_ai']<=predecessor['binary_rates']['pooled_real_false_ai']}
    passed=all(all(v.values()) for v in checks.values())
    result={'state':'E103_limited_DEV_comparison_complete','checks':checks,'passes_limited_dev_screen':passed,
    ...
    write_once(REPORT,result);write_once(EVIDENCE/'e103_development.json',result)

...
from experiments import e62_constrained as model
...
def resource_check(deadline):
    if time.monotonic() > deadline:
        raise TimeoutError('E62 runtime budget exceeded')
...
def freeze():
    ...
    config = {'state': 'E62_constrained_TRAIN_fit_registered', 'inputs': inputs,
    ...
    write_once(EVIDENCE / 'e62_contract.json', config | {'contract_sha256': digest(CONTRACT)})
    return {'state': config['state'], 'contract_sha256': digest(CONTRACT)}


def validate():
    config = read(CONTRACT)
    if digest(CONTRACT) != read(EVIDENCE / 'e62_contract.json')['contract_sha256']:
...
def fit():
    ...
    if RESULT.exists() or CANDIDATE.exists():
        raise FileExistsError('E62 already executed; no refit or replacement')
    ...
    result = {'state': 'E62_TRAIN_feasible' if accepted else 'E62_TRAIN_guard_failed',
    ...
    write_once(RESULT, result); write_once(EVIDENCE / 'e62_fit.json', result)

"""One worst-REAL constrained fit adding frozen supervised E91 features."""
...
if __name__=='__main__':
    def denied(*a,**kw):raise RuntimeError('E92 fit is offline')
...
from experiments.e92_gates import full_training_gates,population_gates as real_slice_gates
from experiments.e91_data import load as load_four,CONDITIONS
from experiments.e91_representation import validate as validate_representation
...
ROOT=DATA_ROOT/'e92';EVIDENCE=ML_ROOT.parent/'evidence'
CONTRACT=ROOT/'fit_contract.json';CANDIDATE=ROOT/'correction.npz';REPORT=ROOT/'fit.json'


def write_report(result):
    write_once(REPORT,result);write_once(EVIDENCE/'e92_fit.json',result)


def freeze():
    ROOT.mkdir(exist_ok=True);validate_representation()
    representation=read(DATA_ROOT/'e91/representation.json');previous_result=read(DATA_ROOT/'e86/dev_report.json')
    if digest(DATA_ROOT/'e91/representation.json')!=digest(EVIDENCE/'e91_representation.json') or \
            representation['state']!='E91_supervised_TRAIN_representation_complete' or representation['epochs']!=100 or \
            representation['TRAIN_views']!=49076 or representation['ai_views']!=18380 or representation['feature_shape']!=[12269,4,64] or \
            representation['conditions']!=CONDITIONS or not representation['previous_four_view_input_replay']['passed'] or \
            ...
            representation['new_image_inference'] or digest(DATA_ROOT/'e91/supervised_map.npz')!=representation['artifact_sha256'] or \
            digest(DATA_ROOT/'e91/supervised_features.npz')!=representation['feature_sha256'] or \
            digest(DATA_ROOT/'e86/dev_report.json')!=digest(EVIDENCE/'e86_development.json') or previous_result['passes_limited_dev_screen']:
        raise ValueError('complete verified E91 and frozen rejected E86 required')
    paths=[DATA_ROOT/'e91/representation_contract.json',DATA_ROOT/'e91/representation.json',
        DATA_ROOT/'e91/supervised_map.npz',DATA_ROOT/'e91/supervised_features.npz',
        DATA_ROOT/'e86/fit_contract.json',DATA_ROOT/'e86/dev_report.json',DATA_ROOT/'e80/correction.npz',
        Path(__file__),Path(model.__file__),Path(__file__).with_name('e83_model.py'),
        Path(__file__).with_name('e91_representation.py'),Path(__file__).with_name('e91_data.py'),
        Path(__file__).with_name('e92_gates.py'),Path(__file__).with_name('e80_fit.py'),
        Path(__file__).with_name('e81_real_minimax.py')]
    old=read(DATA_ROOT/'e86/fit_contract.json')
    ...
    c['limits']+=' Whole SID publisher research TRAIN only; filename/camera groups do not prove scene independence.'
    c.update(state='E92_SID_expanded_supervised_TRAIN_fit_registered',inputs={str(p):digest(p) for p in paths},
        parents=12269,previous_parents=12141,new_parents=128,real_parents=7674,
        ai_views=18380,views=49076,conditions=CONDITIONS,dev_consumed_by=['E70','E71','E83','E86'],
        representation='Same E83 functional450-column layout: exact E80 multimodal385, new frozen E91 supervised64, '
        ...
        constraints='Same per-view E43 confidence guard extended to all18,380 AI views, including missed AI, '
                    'and same correct-REAL decision guard for all four conditions and all legacy/MIDD/SID REAL parents.',
        train_guard='Solver success/violation<=1e-8/minimum AI shift>=-1e-8; exact saved replay; zero newly missed AI '
                    'and zero newly wrong REAL. All4 conditions preserve previous E86 legacy/MIDD population guards, '
                    'add SID pooled<=10%/worst camera<=20%/all REAL<=10%, and all10 absolute numeric gates on legacy, previous and expanded TRAIN '
                    '(120 checks). New-cohort success cannot mask previous failures. Then replay all49,076 '
        ...
        evaluation='Only full TRAIN/runtime pass permits a separately frozen consumed E66 comparison using unchanged '
                   ...
                   'also report paired changes against rejected E86. No E49 until DEV passes.',
        hypothesis='One score-blind128-parent SID TRAIN expansion, same four views/neural architecture/seed/optimizer/head objective. '
                   'No seed, epoch, rank, regularization, margin, weight or cut sweep. This cannot guarantee unseen AI retention.')
    write_once(CONTRACT,c);write_once(EVIDENCE/'e92_fit_contract.json',c|{'contract_sha256':digest(CONTRACT)})
    return {'state':c['state'],'parents':c['parents'],'views':c['views']}


def validate():
    validate_representation();c=read(CONTRACT)
    if digest(CONTRACT)!=read(EVIDENCE/'e92_fit_contract.json')['contract_sha256']:raise ValueError('E92 contract changed')
    for p,s in c['inputs'].items():
        if digest(p)!=s:raise ValueError('E92 input changed: '+p)
    return c


def fit():
    c=validate();torch.set_num_threads(2)
    if CANDIDATE.exists() or REPORT.exists():raise FileExistsError('E92 candidate already fitted')
    ...
    with np.load(DATA_ROOT/'e80/correction.npz',allow_pickle=False) as previous_arrays, \
            np.load(DATA_ROOT/'e91/supervised_map.npz',allow_pickle=False) as representation:
        if str(representation['binding'])!=digest(DATA_ROOT/'e91/representation_contract.json') or \
    ...
    with threadpool_limits(limits=2):
        ...
        with np.load(DATA_ROOT/'e91/supervised_features.npz',allow_pickle=False) as cached:
            if str(cached['binding'])!=digest(DATA_ROOT/'e91/representation_contract.json') or \
        ...
        def check():
            ...
            if counter[0]%10==0:print(json.dumps({'E92_iteration':counter[0],'seconds':round(time.monotonic()-start)}),flush=True)
    ...
    slices=real_slice_gates(rows,candidate.reshape(-1,4),c['old_parents'],c['previous_parents'])
    absolute=full_training_gates(rows,candidate.reshape(-1,4),c['old_parents'],c['previous_parents'])
    ...
    result={'state':'E92_TRAIN_guard_passed' if passed else 'E92_TRAIN_guard_failed','dev_scoring_permitted':bool(passed),
        'solver':solver,'train_gate':gate,'real_population_gates':slices,'full_TRAIN_metric_gates':absolute,
        'runtime_batch_replay':runtime,
        'reference_real_slices':real_slice_gates(rows,baseline.reshape(-1,4),c['old_parents'],c['previous_parents']),

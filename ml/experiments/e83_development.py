"""One consumed E66 DEV comparison after E83 TRAIN feasibility; never an independent final."""
...
if __name__=='__main__':
    def denied(*args,**kwargs):raise RuntimeError('E83 DEV network disabled')
...
from experiments import e42_features as dino,e83_model as model
from experiments.e65_acquisition import digest,read,write_once
from experiments.e65_diagnostic import social_q75_bytes
from experiments.e83_fit import validate as validate_fit,CONTRACT as FIT_CONTRACT,CANDIDATE,REPORT as FIT_REPORT
from experiments.e83_fit import ROOT,EVIDENCE
from experiments.e71_features import resource_check,load_encoder,save_npz
...
FEATURES=ROOT/'dev_features.npz'
...
def freeze():
    ...
    if digest(FIT_REPORT)!=digest(EVIDENCE/'e83_fit.json') or not result['dev_scoring_permitted']:
        raise ValueError('E83 TRAIN guard failed: DEV scoring denied')
    ...
    files=[Path(__file__),Path(model.__file__),Path(dino.__file__),Path(__file__).with_name('e65_diagnostic.py'),
        ...
        Path(dear_model.__file__),Path(__file__).with_name('e80_development.py'),Path(__file__).with_name('e71_development.py'),DATA_ROOT/'e78/dear_r.pth']
    ...
    c={'state':'E83_single_consumed_E66_DEV_comparison_registered','inputs':{str(p):digest(p) for p in files},
       ...
       'features':'Unchanged E43 global+2texture crops for each image/transport, alongside exact official '
                  ...
                  'ViT-L/14 mean/std1536. Exact frozen E80 original/CLIP/bilinear/REAL residual/DEAR map '
                  'plus frozen E82 supervised64 coordinates and E83 constrained head. No fitting. DEAR batch9 padded only for the last partial microbatch; discard padding before three-crop '
                  'mean/std1640. No input resize beyond frozen E43 crop pipeline; encoder outputs float32, '
                  'supervised latent computation CPUfloat64 as frozen in E82. Store all three encoder '
                  'feature arrays externally with ordered image identities and DEVELOPMENT role; never TRAIN.',
    ...
    write_once(CONTRACT,c);write_once(EVIDENCE/'e83_dev_contract.json',c|{'contract_sha256':digest(CONTRACT)})
    return {'state':c['state'],'views':640}


def validate():
    validate_fit();c=read(CONTRACT)
    if digest(CONTRACT)!=read(EVIDENCE/'e83_dev_contract.json')['contract_sha256']:raise ValueError('DEV contract changed')
...
def score():
    c=validate()
    if SCORES.exists() or FEATURES.exists():raise FileExistsError('DEV outputs already exist')
    import torch
    ...
    saved_features={k:[] for k in ('dino','clip','dear')}
    with torch.inference_mode(),threadpool_limits(limits=2):
        for start in range(0,len(rows),c['batch_views']):
            ...
            saved_features['dino'].append(original);saved_features['clip'].append(clip_features);saved_features['dear'].append(forensic)
    ...
    save_npz(FEATURES,**{k:np.concatenate(v) for k,v in saved_features.items()},
        record_ids=np.array([r['record_id'] for r in done]),image_sha256=np.array([r['sha256'] for r in done]),
        roles=np.repeat('DEVELOPMENT',len(done)),binding=np.array(digest(CONTRACT)))
    result={'state':'E83_DEV_scores_locked_before_metrics','contract_sha256':digest(CONTRACT),
        'rows':done,'seconds':time.monotonic()-start_time,'prior_reference_max_error':reference_max_error,
        'previously_consumed_by':['E70','E71'],'reference_decision_changes':0,'features_sha256':digest(FEATURES)}
    write_once(SCORES,result);write_once(EVIDENCE/'e83_dev_scores.json',{'views':len(done),'scores_sha256':digest(SCORES),
        'contract_sha256':digest(CONTRACT),'seconds':result['seconds'],'metrics_opened':False,
        'prior_reference_max_error':reference_max_error,'reference_decision_changes':0,'features_sha256':digest(FEATURES)})
    return {'views':len(done),'seconds':result['seconds']}


def report():
    c=validate()
    if digest(SCORES)!=read(EVIDENCE/'e83_dev_scores.json')['scores_sha256']:raise ValueError('locked DEV scores differ')
    ...
    result={'state':'E83_limited_DEV_comparison_complete','checks':checks,'passes_limited_dev_screen':passed,
    ...
    write_once(REPORT,result);write_once(EVIDENCE/'e83_development.json',result)

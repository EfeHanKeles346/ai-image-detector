...
ROOT=DATA_ROOT/'e121b';EVIDENCE=ML_ROOT.parent/'evidence';CONTRACT=ROOT/'contract.json'
...
def freeze():
    ...
    c={'state':'E121B_posthoc_readonly_transfer_audit_registered','inputs':{str(p):digest(p) for p in files},
        ...
        'engineering_revision':'E121 stopped before report because4278 legacy TRAIN rows use lowercase train. E121B applies the same case-insensitive TRAIN validation as upstream E112; no role reassignment.',
        'predecessor_contract_sha256':digest(DATA_ROOT/'e121/contract.json')}
    ROOT.mkdir(exist_ok=True);write_once(CONTRACT,c);write_once(EVIDENCE/'e121b_transfer_contract.json',c|{'contract_sha256':digest(CONTRACT)})
    return {'paired_consumed_views':1280,'TRAIN_metadata_parents':12525}


def audit():
    c=read(CONTRACT)
    if digest(CONTRACT)!=read(EVIDENCE/'e121b_transfer_contract.json')['contract_sha256']:raise ValueError('Audit contract differs')
    ...
    for r in read(DATA_ROOT/'e112/contract.json')['rows']:
        if str(r['role']).upper()!='TRAIN':raise ValueError('Only admitted TRAIN geometry')
    ...
    result={'state':'E121B_failed_context_transfer_audited','contract_sha256':digest(CONTRACT),
    ...
    write_once(ROOT/'report.json',result);write_once(EVIDENCE/'e121b_context_transfer.json',result)

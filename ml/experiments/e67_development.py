"""One E66 DEV comparison after E67 TRAIN feasibility; never an independent final."""
...
if __name__=='__main__':
    def denied(*args,**kwargs):raise RuntimeError('E67 DEV network disabled')
...
from experiments import e42_features as dino,e67_model as model
from experiments.e65_acquisition import digest,read,write_once
from experiments.e65_diagnostic import social_q75_bytes
from experiments.e67_fit import validate as validate_fit,CONTRACT as FIT_CONTRACT,CANDIDATE,REPORT as FIT_REPORT
from experiments.e67_features import ROOT,EVIDENCE,resource_check
...
def freeze():
    ...
    if digest(FIT_REPORT)!=digest(EVIDENCE/'e67_fit.json') or not result['dev_scoring_permitted']:
        raise ValueError('E67 TRAIN guard failed: DEV scoring denied')
    ...
    c={'state':'E67_single_E66_DEV_comparison_registered','inputs':{str(p):digest(p) for p in files},
       ...
       'features':'Unchanged E43 global+2texture crops for each image/transport, plus GaussianBlur0.8 '
                  'on exactly those crops; no crop reselection. Same order/transform as E67 TRAIN.',
    ...
    write_once(CONTRACT,c);write_once(EVIDENCE/'e67_dev_contract.json',c|{'contract_sha256':digest(CONTRACT)})
    return {'state':c['state'],'views':640}


def validate():
    validate_fit();c=read(CONTRACT)
    if digest(CONTRACT)!=read(EVIDENCE/'e67_dev_contract.json')['contract_sha256']:raise ValueError('DEV contract changed')
...
def score():
    ...
    result={'state':'E67_DEV_scores_locked_before_metrics','contract_sha256':digest(CONTRACT),
        'rows':done,'seconds':time.monotonic()-start_time}
    write_once(SCORES,result);write_once(EVIDENCE/'e67_dev_scores.json',{'views':len(done),'scores_sha256':digest(SCORES),
...
def report():
    c=validate()
    if digest(SCORES)!=read(EVIDENCE/'e67_dev_scores.json')['scores_sha256']:raise ValueError('locked DEV scores differ')
    ...
    result={'state':'E67_limited_DEV_comparison_complete','checks':checks,'passes_limited_dev_screen':passed,
    ...
    write_once(REPORT,result);write_once(EVIDENCE/'e67_development.json',result)

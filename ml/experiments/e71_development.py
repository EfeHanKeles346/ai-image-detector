"""One consumed E66 DEV comparison after E71 TRAIN feasibility; never an independent final."""
...
if __name__=='__main__':
    def denied(*args,**kwargs):raise RuntimeError('E71 DEV network disabled')
...
from experiments import e42_features as dino,e71_model as model
from experiments.e65_acquisition import digest,read,write_once
from experiments.e65_diagnostic import social_q75_bytes
from experiments.e71_fit import validate as validate_fit,CONTRACT as FIT_CONTRACT,CANDIDATE,REPORT as FIT_REPORT
from experiments.e71_fit import ROOT,EVIDENCE
...
def clip_aggregate(raw):
    if raw.shape!=(3,768) or raw.dtype!=np.float32 or not np.isfinite(raw).all():
        raise ValueError('invalid CLIP crop vectors')
    return np.concatenate([raw.mean(axis=0),raw.std(axis=0,ddof=0)]).astype(np.float32)


def check_reference(row,score,previous,tolerance):
    if previous['sha256']!=row['sha256']:raise ValueError('historical DEV pixel identity changed')
    error=abs(float(score)-previous['reference_score'])
    if not np.isfinite(score) or error>tolerance or any((score>=cut)!=(previous['reference_score']>=cut)
                                                     for cut in [model.AI_CUT,model.REAL_CUT]):
        raise ValueError('historical reference score or decision changed')
    return error
...
def freeze():
    ...
    if digest(FIT_REPORT)!=digest(EVIDENCE/'e71_fit.json') or not result['dev_scoring_permitted']:
        raise ValueError('E71 TRAIN guard failed: DEV scoring denied')
    ...
    files=[Path(__file__),Path(model.__file__),Path(dino.__file__),Path(__file__).with_name('e65_diagnostic.py'),
        ...
        Path(__file__).with_name('e71_features.py'), DATA_ROOT/'e70/dev_scores.json', DATA_ROOT/'e70/dev_report.json']
    c={'state':'E71_single_consumed_E66_DEV_comparison_registered','inputs':{str(p):digest(p) for p in files},
       'manifest':fit['dev_manifest'],'manifest_sha256':fit['dev_manifest_sha256'],
       'reference':fit['inputs']['reference'],'views':640,'batch_views':8,'max_seconds':2400,
       ...
       'features':'Unchanged E43 global+2texture crops for each image/transport, alongside exact official '
                  ...
                  'ViT-L/14 mean/std1536. E71 original64/CLIP64 map and fixed correction weights.',
       'dev_status':'CONSUMED DEVELOPMENT: E70 scores and report already exposed. Not fresh validation.',
    ...
    write_once(CONTRACT,c);write_once(EVIDENCE/'e71_dev_contract.json',c|{'contract_sha256':digest(CONTRACT)})
    return {'state':c['state'],'views':640}


def validate():
    validate_fit();c=read(CONTRACT)
    if digest(CONTRACT)!=read(EVIDENCE/'e71_dev_contract.json')['contract_sha256']:raise ValueError('DEV contract changed')
...
def score():
    ...
    prior_rows=read(DATA_ROOT/'e70/dev_scores.json')['rows']
    ...
    with torch.inference_mode(),threadpool_limits(limits=2):
        for start in range(0,len(rows),c['batch_views']):
            ...
            old=head.predict_proba(original)[:,1];new=model.predict(head,original,clip_features,arrays)
    ...
    result={'state':'E71_DEV_scores_locked_before_metrics','contract_sha256':digest(CONTRACT),
        'rows':done,'seconds':time.monotonic()-start_time,'prior_reference_max_error':reference_max_error,
        'previously_consumed_by':['E70'],'reference_decision_changes':0}
    write_once(SCORES,result);write_once(EVIDENCE/'e71_dev_scores.json',{'views':len(done),'scores_sha256':digest(SCORES),
...
def report():
    c=validate()
    if digest(SCORES)!=read(EVIDENCE/'e71_dev_scores.json')['scores_sha256']:raise ValueError('locked DEV scores differ')
    ...
    result={'state':'E71_limited_DEV_comparison_complete','checks':checks,'passes_limited_dev_screen':passed,
    ...
    write_once(REPORT,result);write_once(EVIDENCE/'e71_development.json',result)

...
from pixelproof import source_pixel_views_b as views, source_pixel_residual
from pixelproof.project_paths import DATA_ROOT, ML_ROOT

ROOT=DATA_ROOT/'e150b'; EVIDENCE=ML_ROOT.parent/'evidence'
...
def freeze():
    ...
    if digest(DATA_ROOT/'e150/probe_failure.json')!=digest(EVIDENCE/'e150_source_pixel_probe_failure.json'):
        raise ValueError('Frozen failed E150 receipt required')
    ...
    files=[Path(__file__),Path(views.__file__),Path(source_pixel_residual.__file__),Path(previous.__file__),
           ...
           ML_ROOT/'src/pixelproof/source_pixel_views.py',ML_ROOT/'experiments/e150_source_pixel_features.py',
           DATA_ROOT/'e150/probe_contract.json',DATA_ROOT/'e150/probe_failure.json',EVIDENCE/'e150_source_pixel_probe_failure.json',
    ...
    c=dict(state='E150B_four_condition_probe_registered',inputs=prior['inputs']|{str(p):digest(p) for p in files},
        ...
        implementation='Exact frozen E150 pixels/features, but in-place EXIF on the private source, no repeated full-RGB copies for128 crops, directly retain loaded RGB JPEG frames. Memory guard remains1536MiB; no threshold or feature change.',
    ...
    write_once(EVIDENCE/'e150b_source_pixel_probe_contract.json',{k:v for k,v in c.items() if k not in ('inputs','selected')}|{'contract_sha256':digest(CONTRACT)})
    return {'probe_parents':c['probe_parents'],'geometry':geometry,'contract_sha256':digest(CONTRACT)}


def validate():
    c=read(CONTRACT)
    if digest(CONTRACT)!=read(EVIDENCE/'e150b_source_pixel_probe_contract.json')['contract_sha256'] or c['versions']!=previous.versions():
...
def probe():
    ...
    with threadpool_limits(limits=2):
        for i,parent in enumerate(c['selected']):
            ...
            values.append(value)
            if (i+1)%8==0:print(json.dumps({'E150B_probe':i+1,'seconds':round(time.monotonic()-start)}),flush=True)
    ...
    report=dict(state='E150B_four_condition_probe_complete',contract_sha256=digest(CONTRACT),parents=len(values),
    ...
    write_once(ROOT/'probe.json',report);write_once(EVIDENCE/'e150b_source_pixel_probe.json',report)
    return report


def register_full():
    c=validate();p=read(ROOT/'probe.json')
    if digest(ROOT/'probe.json')!=digest(EVIDENCE/'e150b_source_pixel_probe.json') or not p['passes_full_resource_gate'] or \
    ...
    full=dict(state='E150B_full_source_pixel_features_registered',probe_contract_sha256=digest(CONTRACT),
    ...
    write_once(FULL,full);write_once(EVIDENCE/'e150b_source_pixel_full_contract.json',full|{'contract_sha256':digest(FULL)})
    return full
...
def extract():
    ...
    if binding!=read(EVIDENCE/'e150b_source_pixel_full_contract.json')['contract_sha256'] or \
    ...
    with threadpool_limits(limits=2):
        for offset in range(0,len(rows),full['chunk_parents']):
            ...
            print(json.dumps({'E150B_parents':offset+len(part),'created':created,'resumed':resumed,'seconds':round(time.monotonic()-start)}),flush=True)
    ...
    report=dict(state='E150B_full_source_pixel_features_complete',contract_sha256=binding,parents=len(rows),conditions=views.CONDITIONS,
    ...
    write_once(EVIDENCE/'e150b_source_pixel_features.json',public)
    return public

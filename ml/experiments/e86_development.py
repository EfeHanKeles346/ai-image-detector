"""One consumed E66 comparison from frozen encoder caches after full E86 TRAIN/runtime pass."""
...
if __name__=='__main__':
    def denied(*a,**kw):raise RuntimeError('E86 consumed DEV offline')
...
from experiments.e86_fit import validate as validate_fit,CONTRACT as FIT_CONTRACT,CANDIDATE,REPORT as FIT_REPORT,ROOT,EVIDENCE
...
def freeze():
    ...
    if digest(FIT_REPORT)!=digest(EVIDENCE/'e86_fit.json') or not fit['dev_scoring_permitted']:
        raise ValueError('E86 complete TRAIN/runtime pass required before DEV cache access')
    ...
    paths=[Path(__file__),Path(model.__file__),FIT_CONTRACT,FIT_REPORT,CANDIDATE,FEATURES,CACHE_CONTRACT,CACHE_SCORES,
    ...
    c={'state':'E86_single_consumed_E66_cache_comparison_registered','inputs':{str(p):digest(p) for p in paths},
       ...
       'features':'Exact complete E83 DEVELOPMENT DINO3072/CLIP1536/DEAR1640 arrays, ordered original/Q75 image '
                  ...
                  'Frozen E86 map in runtime batch8. Replay every E43 reference score<=1e-6 and both cuts unchanged.',
       'dev_status':'CONSUMED DEVELOPMENT already used by E70/E71/E83. Neither fresh nor independent final.',
       'comparison':'All20 original numeric gates, zero newly missed E43-caught AI in every source/condition, '
                    ...
                    'against rejected E83 so losing its rescues cannot be hidden. Scores locked before metrics.',
       'limits':cache['limits']+' E83 labels/features remain excluded from TRAIN; cached comparison does not reset consumption.',
    ...
    write_once(CONTRACT,c);write_once(EVIDENCE/'e86_dev_contract.json',c|{'contract_sha256':digest(CONTRACT)})
    return {'state':c['state'],'views':c['views']}


def validate():
    validate_fit();validate_cache();c=read(CONTRACT)
    if digest(CONTRACT)!=read(EVIDENCE/'e86_dev_contract.json')['contract_sha256']:raise ValueError('E86 DEV contract changed')
    for p,s in c['inputs'].items():
        if digest(p)!=s:raise ValueError('E86 DEV input changed: '+p)
...
def score():
    c=validate()
    if SCORES.exists():raise FileExistsError('E86 DEV scores already locked')
    torch.set_num_threads(2);start=time.monotonic();deadline=start+c['max_seconds'];resource_check(deadline)
    rows=read(CACHE_SCORES)['rows'];validate_pairs(rows,read(c['manifest'])['rows'])
    ...
    result={'state':'E86_DEV_scores_locked_before_metrics','contract_sha256':digest(CONTRACT),'rows':done,
            'prior_reference_max_error':max_error,'reference_decision_changes':0,'features_sha256':digest(FEATURES),
            'image_reads':0,'seconds':time.monotonic()-start,'previously_consumed_by':['E70','E71','E83']}
    write_once(SCORES,result);write_once(EVIDENCE/'e86_dev_scores.json',{k:v for k,v in result.items() if k!='rows'}|
...
def report():
    c=validate()
    if digest(SCORES)!=read(EVIDENCE/'e86_dev_scores.json')['scores_sha256']:raise ValueError('locked E86 scores changed')
    ...
    for condition in CONDITIONS:
        ...
        reports[condition]={'old':old,'new':new,'transitions':change,'real_scenes':scenes,
                            'E83_predecessor':predecessor,'transitions_vs_E83':previous_changes}
    ...
    result={'state':'E86_limited_DEV_comparison_complete','checks':checks,'passes_limited_dev_screen':passed,
    ...
    write_once(REPORT,result);write_once(EVIDENCE/'e86_development.json',result)

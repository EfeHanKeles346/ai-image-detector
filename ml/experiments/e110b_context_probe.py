"""E110B: identical E110 probe with verified historical source-key materialization."""
import argparse
import hashlib
import json
import os
...
from experiments.e84_sources import materialized_rows, INDEX, REPORT, CONTRACT as SOURCES_CONTRACT

ROOT = DATA_ROOT/'e110b'; EVIDENCE = ML_ROOT.parent/'evidence'; CONTRACT = ROOT/'contract.json'
...
def population():
    ...
    resolved = materialized_rows()
    if [r['parent_id'] for r in resolved] != [r['parent_id'] for p in parts[:2] for r in p]:
        raise ValueError('Historical materialized parent order differs')
    parts[:2] = [resolved[:11630], resolved[11630:]]
...
def freeze():
    ...
    previous = read(DATA_ROOT/'e110/contract.json')
    if [r['parent_id'] for r in rows] != [r['parent_id'] for r in previous['selected']]:
        raise ValueError('Probe selection changed')
    files = [Path(__file__), ML_ROOT/'experiments/e110_context_probe.py',
             DATA_ROOT/'e110/contract.json', INDEX, REPORT, SOURCES_CONTRACT,
             ML_ROOT/'experiments/e84_sources.py', Path(load_encoder.__code__.co_filename), Path(crop_views.__code__.co_filename),
    ...
    c = {'state': 'E110B_missing_raw_TRAIN_probe_registered',
    ...
    write_once(EVIDENCE/'e110b_context_contract.json', {k:v for k,v in c.items() if k != 'selected'} |
...
def run():
    import torch
    c = read(CONTRACT)
    if digest(CONTRACT) != read(EVIDENCE/'e110b_context_contract.json')['contract_sha256'] or digest(BACKBONE) != c['backbone_sha256']:
    ...
    result = {'state': 'E110B_missing_raw_TRAIN_probe_complete', 'contract_sha256': digest(CONTRACT),
    ...
    write_once(ROOT/'probe.json', result); write_once(EVIDENCE/'e110b_context_probe.json', result)
    return result

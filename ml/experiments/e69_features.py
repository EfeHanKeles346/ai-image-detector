"""TRAIN-only E69 fixed-crop shuffle response features; restartable and offline."""
...
if __name__=='__main__':
    def denied(*args, **kwargs):raise RuntimeError('E69 feature network disabled')
...
OUTPUT=ROOT/'shuffle_features.npz'
...
PATCH_SIZE=28
PERMUTATION=np.random.default_rng(69).permutation(64)


def shuffle_crops(crops):
    if crops.dtype!=np.uint8 or crops.shape!=(3,3,224,224,3):
        raise ValueError('expected three conditions x three uint8 RGB crops')
    patches=crops.reshape(9,8,28,8,28,3).transpose(0,1,3,2,4,5).reshape(9,64,28,28,3)
    return patches[:,PERMUTATION].reshape(9,8,8,28,28,3).transpose(0,1,3,2,4,5).reshape(crops.shape).copy()


def resource_check(deadline):
    if time.monotonic()>deadline:raise TimeoutError('E69 feature runtime budget reached')
...
def freeze():
    ...
    c={'state':'E69_fixed_patch_shuffle_TRAIN_features_registered','inputs':inputs,'parents':11630,
       'conditions':['clean','assigned_transport','q75'],'crop_shape':[3,3,224,224,3],
       'transform':'Fixed NumPy default_rng(seed69) permutation of64 non-overlapping28px patches per RGB224 crop, '
                   'all labels and all3 cached E54 conditions identically. No crop reselection.',
       'feature':'Frozen DINOv2S blocks2/5/8/11 CLS, mean/std over3 crops; store shuffled3072D. '
                 'Candidate uses original PCA64 plus standardized shuffled-feature PCA64, TRAIN only.',
       'patch_size':28,'permutation':PERMUTATION.tolist(),'batch_parents':8,'chunk_parents':128,'max_seconds':14400,'cpu_threads':2,
    ...
    write_once(EVIDENCE/'e69_features_contract.json',c|{'contract_sha256':digest(CONTRACT)})
    return {'state':c['state'],'parents':len(rows),'reference_parity_parents':len(parity)}


def digest_string(value):
    import hashlib
    return hashlib.sha256(('E69|'+value).encode()).hexdigest()


def validate():
    c=read(CONTRACT)
    if digest(CONTRACT)!=read(EVIDENCE/'e69_features_contract.json')['contract_sha256']:
...
def load_crops(record,binding,shuffle=False):
    ...
    return shuffle_crops(crops) if shuffle else crops


def extract():
    c=validate()
    if REPORT.exists() or OUTPUT.exists():raise FileExistsError('E69 feature extraction already complete')
    import torch
        ...
        for start in range(0,len(rows),c['chunk_parents']):
            ...
            if receipt.exists():
                ...
                for offset in range(0,len(group),c['batch_parents']):
                    ...
                    if offset%32==0:print(json.dumps({'stage':'shuffle_features','parents':start+offset+len(batch),
    ...
    result={'state':'E69_TRAIN_shuffle_features_complete','contract_sha256':binding,'feature_sha256':digest(OUTPUT),
    ...
    write_once(REPORT,result);write_once(EVIDENCE/'e69_features.json',result|{'report_sha256':digest(REPORT)})
    return result

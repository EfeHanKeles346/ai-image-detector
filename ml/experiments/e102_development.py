"""One consumed E66 comparison from frozen encoder caches after full E102 TRAIN/runtime pass."""
...
if __name__=='__main__':
    def denied(*a,**kw):raise RuntimeError('E102 consumed DEV offline')
...
from experiments.e102_fit import validate as validate_fit,CONTRACT as FIT_CONTRACT,CANDIDATE,REPORT as FIT_REPORT,ROOT,EVIDENCE
...
PREVIOUS_SCORES=DATA_ROOT/'e92/dev_scores.json';PREVIOUS_REPORT=DATA_ROOT/'e92/dev_report.json'
...
def validate_predecessor(rows, cached, manifest):
    ...
    if any(any(a[k]!=b[k] for k in fields) for a,b in zip(rows,cached,strict=True)):
        raise ValueError('E92 predecessor and E83 cache identities/order differ')


def freeze():
    validate_fit();fit=read(FIT_REPORT)
    if digest(FIT_REPORT)!=digest(EVIDENCE/'e102_fit.json') or not fit['dev_scoring_permitted']:
        raise ValueError('E102 complete TRAIN/runtime pass required before DEV cache access')
    ...
    if digest(PREVIOUS_REPORT)!=digest(EVIDENCE/'e92_development.json') or digest(PREVIOUS_SCORES)!=previous['scores_sha256']:
        raise ValueError('immutable E92 predecessor report/scores required')
    ...
    c={'state':'E102_single_consumed_E66_cache_comparison_registered','inputs':{str(p):digest(p) for p in paths},
       ...
       'features':'Exact complete E83 DEVELOPMENT DINO3072/CLIP1536/DEAR1640 arrays, ordered original/Q75 image '
                  ...
                  'Frozen E102 map in runtime batch8. Replay every E43 reference score<=1e-6 and both cuts unchanged.',
       'dev_status':'CONSUMED DEVELOPMENT already used by E70/E71/E83/E92. Neither fresh nor independent final.',
       'comparison':'All20 numeric gates; zero lost E43 or E92 AI and zero new E92 REAL errors in each source/condition. '
                    'non-increased pooled REAL FPR in both conditions. Also report all paired source transitions '
                    'against rejected E92 so losing its rescues cannot be hidden. Scores locked before metrics.',
       'limits':cache['limits']+' E83/E92 DEV labels/features remain excluded from TRAIN; cached comparison does not reset consumption.',
    ...
    write_once(CONTRACT,c);write_once(EVIDENCE/'e102_dev_contract.json',c|{'contract_sha256':digest(CONTRACT)})
    return {'state':c['state'],'views':c['views']}


def validate():
    validate_fit();validate_cache();c=read(CONTRACT)
    if digest(CONTRACT)!=read(EVIDENCE/'e102_dev_contract.json')['contract_sha256']:raise ValueError('E102 DEV contract changed')
    for p,s in c['inputs'].items():
        if digest(p)!=s:raise ValueError('E102 DEV input changed: '+p)
...
def score():
    c=validate()
    if SCORES.exists():raise FileExistsError('E102 DEV scores already locked')
    ...
    result={'state':'E102_DEV_scores_locked_before_metrics','contract_sha256':digest(CONTRACT),'rows':done,
            'prior_reference_max_error':max_error,'reference_decision_changes':0,'features_sha256':digest(FEATURES),
            'image_reads':0,'seconds':time.monotonic()-start,'previously_consumed_by':['E70','E71','E83','E86','E92']}
    write_once(SCORES,result);write_once(EVIDENCE/'e102_dev_scores.json',{k:v for k,v in result.items() if k!='rows'}|
...
def report():
    c=validate()
    if digest(SCORES)!=read(EVIDENCE/'e102_dev_scores.json')['scores_sha256']:raise ValueError('locked E102 scores changed')
    ...
    for condition in CONDITIONS:
        ...
        reports[condition]={'old':old,'new':new,'transitions':change,'real_scenes':scenes,
                            'E92_predecessor':predecessor,'transitions_vs_E92':previous_changes}
        checks[condition]={'absolute_gates_passed':new['gate']['passed'],
            'zero_new_ai_misses':all(v['new_ai_misses']==0 for v in change.values()),
            'zero_lost_E92_ai':all(v['new_ai_misses']==0 for v in previous_changes.values()),
            'zero_new_E92_real_errors':all(v['new_real_false_ai']==0 for v in previous_changes.values()),
    ...
    result={'state':'E102_limited_DEV_comparison_complete','checks':checks,'passes_limited_dev_screen':passed,
    ...
    write_once(REPORT,result);write_once(EVIDENCE/'e102_development.json',result)

"""Bind and score only E48 FIT+CAL with four frozen detector arms."""
...
ROOT = DATA_ROOT / "e48"
MANIFEST_SHA256 = "1404a3ff5cfde4a343e1368b995ab1ec13135811c569134dfffaabf40c1b5b68"
...
CONTRACT = ROOT / "fitcal_score_contract.json"
CONTRACT_EVIDENCE = ML_ROOT.parent / "evidence" / "e48_fitcal_score_contract.json"
ARM_PATHS = {
    "generalist": ROOT / "fitcal_generalist_scores.jsonl",
    "dda": ROOT / "fitcal_dda_scores.jsonl",
    "univfd": ROOT / "fitcal_univfd_scores.jsonl",
    "unina": ROOT / "fitcal_unina_scores.jsonl",
}
...
def _manifest_rows() -> list[dict[str, Any]]:
    ...
    if hashlib.sha256(raw).hexdigest() != MANIFEST_SHA256:
        raise ValueError("E48 manifest changed")
    payload = json.loads(raw)
    rows = payload.get("rows", [])
    if (payload.get("state") != "e48_decontaminated_frozen_unscored"
            or payload.get("model_scores_created") != 0 or len(rows) != 2_400):
        raise ValueError("E48 manifest contract changed")
    return rows


def _fitcal_rows() -> list[dict[str, Any]]:
    rows = [row for row in _manifest_rows() if row["role"] in {"FIT", "CAL"}]
    if (len(rows) != 1_200 or sum(row["role"] == "FIT" for row in rows) != 600
            or sum(row["role"] == "CAL" for row in rows) != 600):
        raise ValueError("E48 FIT+CAL role boundary changed")
    return rows


def bind() -> dict[str, Any]:
    if CONTRACT.exists() or CONTRACT_EVIDENCE.exists():
        raise FileExistsError("E48 FIT+CAL score contract already exists")
    ...
    identities = {
        ...
        "univfd_backbone_sha256": _digest(DATA_ROOT / "e47" / "models" / "ViT-L-14.pt"),
    ...
    expected = {
        "manifest_sha256": MANIFEST_SHA256, "generalist_sha256": GENERALIST_SHA256,
        "dda_sha256": CHECKPOINT_SHA256, "fusion_sha256": FUSION_SHA256,
        "univfd_backbone_sha256": UNIVFD_BACKBONE_SHA256,
        "univfd_head_sha256": UNIVFD_HEAD_SHA256, "unina_sha256": UNINA_SHA256,
    }
    if identities != expected:
        raise ValueError(f"E48 frozen detector identity changed: {identities}")
    payload = {
        "schema_version": 1, "state": "e48_fitcal_score_contract_frozen_before_model_load",
        "identities": identities,
        "counts": {"manifest": len(all_rows), "fitcal": len(rows), "FIT": 600, "CAL": 600,
                   "DEVELOPMENT_unscored": 1_200},
        "roles_scored": ["FIT", "CAL"], "roles_forbidden": ["DEVELOPMENT"],
        "arms": {
            "generalist": "unchanged E43-S clean transport",
            "dda": "official DDA preprocessing/probability",
            "univfd": "official CLIP ViT-L/14 center crop + sigmoid head",
            "unina": f"official StyleGAN2 ResNet50-NoDown after frozen {LONG_SIDE_CAP}px long-side cap",
        },
        "forbidden": ["DEVELOPMENT scoring before candidate freeze", "role change",
                      "backbone update", "failed-row removal", "score-dependent replacement"],
        "model_scores_created": 0, "development_scores_created": 0,
    }
...
def _validate() -> list[dict[str, Any]]:
    ...
    if (hashlib.sha256(raw).hexdigest() != receipt.get("contract_sha256")
        ...
        raise ValueError("E48 FIT+CAL score contract changed")
    return _fitcal_rows()
...
def _resume(path: Path, rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    if raw and not raw.endswith(b"\n"):
        raise ValueError("E48 score partial line truncated")
    scored = [json.loads(line) for line in raw.splitlines() if line]
    if len(scored) > len(rows):
        raise ValueError("E48 score prefix exceeds FIT+CAL")
    for index, item in enumerate(scored):
        ...
        if (item.get("record_id") != row["record_id"] or item.get("role") != row["role"]
                ...
                or item.get("role") == "DEVELOPMENT"
                or not np.isfinite(float(item.get("score", np.nan)))):
            raise ValueError(f"E48 score prefix changed at {index}")
    return scored
...
def _finish(arm: str, rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    if len(scored) != len(rows):
        raise ValueError(f"E48 {arm} FIT+CAL score stream incomplete")
    partial.replace(ARM_PATHS[arm])
    raw = ARM_PATHS[arm].read_bytes()
    evidence = {
        "schema_version": 1, "state": f"e48_fitcal_{arm}_scores_complete",
        "rows": len(rows), "FIT": 600, "CAL": 600, "DEVELOPMENT": 0,
        "coverage": 1.0, "bytes": len(raw), "sha256": hashlib.sha256(raw).hexdigest(),
        "manifest_sha256": MANIFEST_SHA256,
    }
    _write(ML_ROOT.parent / "evidence" / f"e48_fitcal_{arm}_scores.json", evidence)
    return evidence
...
def score_generalist(batch_size: int = 16) -> dict[str, Any]:
    rows = _validate(); partial = ARM_PATHS["generalist"].with_suffix(".jsonl.partial"); done = _resume(partial, rows)
    ...
    with torch.inference_mode(), ThreadPoolExecutor(max_workers=6) as pool:
        for start in range(len(done), len(rows), batch_size):
            ...
            if len(done) % 100 == 0 or len(done) == len(rows): print(f"E48 generalist {len(done)}/{len(rows)}", flush=True)
...
def score_dda(batch_size: int = 8) -> dict[str, Any]:
    ...
    with torch.inference_mode():
        for start in range(len(done), len(rows), batch_size):
            ...
            output = [_base(row, value) for row, value in zip(group, values, strict=True)]
            _append(partial, output); done.extend(output)
            if len(done) % 100 == 0 or len(done) == len(rows): print(f"E48 DDA {len(done)}/{len(rows)}", flush=True)
...
def score_univfd(batch_size: int = 8) -> dict[str, Any]:
    ...
    with torch.inference_mode():
        for start in range(len(done), len(rows), batch_size):
            ...
            output = [_base(row, value) for row, value in zip(group, values, strict=True)]
            _append(partial, output); done.extend(output)
            if len(done) % 100 == 0 or len(done) == len(rows): print(f"E48 UnivFD {len(done)}/{len(rows)}", flush=True)
...
def score_unina(batch_size: int = 4) -> dict[str, Any]:
    ...
    with torch.inference_mode():
        while len(done) < len(rows):
            start = len(done); tensors = []; group = []; target_size = None
            ...
            output = [_base(row, value) for row, value in zip(group, values, strict=True)]
            _append(partial, output); done.extend(output)
            if len(done) % 100 == 0 or len(done) == len(rows): print(f"E48 UNINA {len(done)}/{len(rows)}", flush=True)

...
DATA_CONTRACT = DATA_ROOT / 'e54/data_contract_v2.json'
TEACHER = DATA_ROOT / 'e54/teacher.npz'
INDEX = DATA_ROOT / 'e54/crop_index.json'
...
CONTRACT = ROOT / 'audit_contract_v2.json'
...
def freeze():
    ...
    # Read/hash the prior admission chain; do not import acquisition helpers (Kaggle
    # authenticates at import time even when no transfer function is called).
    for rel, receipt in [('e53/contract_v1.json', 'e53_contract.json'),
                         ('e53/expansion_contract.json', 'e53_expansion_contract.json'),
                         ('e53/coverage_v2/contract.json', 'e53_coverage_contract.json')]:
        p = DATA_ROOT / rel
        verified(p, read(EVIDENCE / receipt)['contract_sha256'])
    ...
    write(EVIDENCE / 'e60_audit_contract_v2.json', value | {'contract_sha256': digest(CONTRACT)})
...
def verify_contract():
    ...
    verified(CONTRACT, read(EVIDENCE / 'e60_audit_contract_v2.json')['contract_sha256'])
...
def run():
    ...
    rows = data['rows']
    if len(rows) != 11630 or any(str(r['role']).upper() != 'TRAIN' or r['label'] not in (0, 1) for r in rows):

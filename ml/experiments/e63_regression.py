"""One consumed E49 comparison of immutable E63; no fresh-final or tuning claim."""
...
from experiments import e63_constrained as correction
from experiments.e63_run import ROOT, EVIDENCE, CONTRACT, CANDIDATE, RESULT, validate, resource_check
...
def freeze():
    ...
    if not fitted['consumed_regression_permitted']:
        raise ValueError('E63 TRAIN guard failed; evaluation denied')
    ...
    config = {'state': 'E63_consumed_E49_comparison_registered',
              ...
              'intervals': '20000 paired source-cluster resamples stratified by label; seed63; '
    ...
    write(EVIDENCE / 'e63_regression_contract.json', config | {'regression_contract_sha256': digest(REGRESSION_CONTRACT)})
...
def regression_inputs():
    ...
    verified(REGRESSION_CONTRACT, read(EVIDENCE / 'e63_regression_contract.json')['regression_contract_sha256'])
...
def regression_score():
    ...
    with np.load(CANDIDATE, allow_pickle=False) as a:
        ...
        weights = {k: a[k] for k in ('weights', 'mean', 'components', 'scales', 'centers', 'sigma', 'kernel_mean', 'kernel_scales')}
    ...
    with torch.inference_mode(), ThreadPoolExecutor(max_workers=4) as pool, threadpool_limits(limits=2):
        for start in range(len(done), len(rows), c['batch_views']):
            ...
            if len(done) % 160 == 0:
                print(json.dumps({'phase': 'E63_frozen_E49_scores', 'rows': len(done), 'total': len(rows),
    ...
    write(REGRESSION_SCORES, {'state': 'E63_scores_locked_before_metrics', 'rows': done,
    ...
    write(EVIDENCE / 'e63_regression_scores.json', {'rows': len(done), 'sha256': digest(REGRESSION_SCORES),
...
def regression_report():
    ...
    verified(REGRESSION_SCORES, read(EVIDENCE / 'e63_regression_scores.json')['sha256'])
    ...
    rng = np.random.default_rng(63); samples = []
    for y in (0, 1):
    ...
    result = {'state': 'E63_consumed_E49_regression_complete_not_fresh_final', 'checks': checks,
    ...
    write(REGRESSION_RESULT, result); write(EVIDENCE / 'e63_regression.json', result)

"""One E69 TRAIN fit after the limited E66 DEV population is frozen, before DEV scoring."""
...
if __name__=='__main__':
    def denied(*args,**kwargs):raise RuntimeError('E69 fit network disabled')
...
from experiments import e69_model as model
from experiments.e69_features import ROOT,EVIDENCE,OUTPUT as SHUFFLE,REPORT as FEATURES_REPORT,resource_check
...
def freeze():
    ...
    if digest(FEATURES_REPORT)!=read(EVIDENCE/'e69_features.json')['report_sha256'] or feature['reference_decision_changes']:
        raise ValueError('unverified shuffle feature extraction')
    inputs['shuffle']={'path':str(SHUFFLE),'sha256':feature['feature_sha256']}
    files={'features_report':FEATURES_REPORT,'feature_contract':ROOT/'features_contract.json',
           ...
           'feature_code':Path(__file__).with_name('e69_features.py'),
    ...
    c={'state':'E69_original_plus_shuffled_texture_TRAIN_fit_registered','inputs':inputs,
       ...
       'representation':'E43-scaled original TRAIN PCA64(seed62,power3), plus TRAIN StandardScaler on '
                        'fixed-crop-shuffled features followed by PCA64(seed69,power3). '
    ...
    write_once(CONTRACT,c);write_once(EVIDENCE/'e69_fit_contract.json',c|{'contract_sha256':digest(CONTRACT)})
    return {'state':c['state'],'dev_manifest_sha256':expected}


def validate():
    c=read(CONTRACT)
    if digest(CONTRACT)!=read(EVIDENCE/'e69_fit_contract.json')['contract_sha256']:raise ValueError('E69 fit contract changed')
    for v in c['inputs'].values():
        if digest(v['path'])!=v['sha256']:raise ValueError('E69 fit input changed')
...
def fit():
    c=validate()
    if CANDIDATE.exists() or REPORT.exists():raise FileExistsError('E69 candidate already fitted')
    ...
    with np.load(p['shuffle'],allow_pickle=False) as a:
        if str(a['binding'])!=digest(p['feature_contract']) or a['features'].shape!=(11630,3,3072) or \
            list(a['parent_ids'])!=[r['parent_id'] for r in rows] or set(a['roles'])!={'TRAIN'}:raise ValueError('shuffle alignment/role differs')
        shuffled=a['features'].reshape(-1,3072)
    ...
    with threadpool_limits(limits=2):
        ...
        print(json.dumps({'stage':'E69_fit_TRAIN_two_bases','views':len(labels)}),flush=True)
        arrays,basis=model.fit_basis(head,original,shuffled)
        x=model.project(head,original,shuffled,arrays)
        if not np.array_equal(baseline,model.predict(head,original,shuffled,arrays)):raise ValueError('zero-init parity failed')
        ...
        def check():
            ...
            if count[0]%10==0:print(json.dumps({'stage':'E69_constrained_fit','iteration':count[0],
                                               'seconds':round(time.monotonic()-started)}),flush=True)
        arrays['weights'],solver=model.fit(x,logits,labels,weights,check)
        candidate=model.predict(head,original,shuffled,arrays)
        ...
        with np.load(CANDIDATE,allow_pickle=False) as a:
            if not np.array_equal(candidate,model.predict(head,original,shuffled,a)):raise ValueError('serialized replay differs')
    ...
    result={'state':'E69_TRAIN_guard_passed' if passed else 'E69_TRAIN_guard_failed','dev_scoring_permitted':bool(passed),
    ...
    write_once(REPORT,result);write_once(EVIDENCE/'e69_fit.json',result)

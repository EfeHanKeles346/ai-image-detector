"""One original/CLIP bilinear TRAIN fit under the E73 full-AI-confidence constraint."""
...
if __name__=='__main__':
    def denied(*a,**kw):raise RuntimeError('E74 fit is offline')
...
def freeze():
    ...
    files=[Path(__file__),Path(model.__file__),Path(__file__).with_name('e64_constrained.py'),Path(__file__).with_name('e73_fit.py'),
        Path(__file__).with_name('e70_model.py'),Path(__file__).with_name('e71_model.py'),
        DATA_ROOT/'e73/fit_contract.json',DATA_ROOT/'e73/fit.json',
    ...
    rejected=read(DATA_ROOT/'e73/fit.json')
    if digest(DATA_ROOT/'e73/fit.json')!=digest(EVIDENCE/'e73_fit.json') or rejected['dev_scoring_permitted']:
        raise ValueError('frozen failed E73 required')
    c={'state':'E74_bilinear_full_AI_confidence_TRAIN_fit_registered','inputs':{str(p):digest(p) for p in files},
       'previous_fit_contract_sha256':digest(DATA_ROOT/'e71/fit_contract.json'),
       'representation':'Exact E71 original64 and CLIP64 TRAIN coordinates plus128D bilinear TensorSketch '
                        '(exact tested E70 construction,seed70), TRAIN StandardScaler on interactions. '
                        'Zero257 coefficients; discard E71 weights. No basis refit or E72 data.',
    ...
    write_once(CONTRACT,c);write_once(EVIDENCE/'e74_fit_contract.json',c|{'contract_sha256':digest(CONTRACT)})
    return {'state':c['state'],'ai_views':13785}


def validate():
    validate_previous();c=read(CONTRACT)
    if digest(CONTRACT)!=read(EVIDENCE/'e74_fit_contract.json')['contract_sha256']:
        raise ValueError('E74 contract changed')
    for p,s in c['inputs'].items():
        if digest(p)!=s:raise ValueError('E74 input changed: '+p)
    return c


def fit():
    c=validate()
    if CANDIDATE.exists() or REPORT.exists():raise FileExistsError('E74 candidate already fitted')
    ...
    with np.load(DATA_ROOT/'e71/correction.npz',allow_pickle=False) as a:
        previous_arrays={k:a[k] for k in model.KEYS}
    ...
    with threadpool_limits(limits=2):
        ...
        arrays=model.fit_map(head,original,clip,previous_arrays)
        ...
        def check():
            ...
            if counter[0]%10==0:print(json.dumps({'E74_iteration':counter[0],'seconds':round(time.monotonic()-start)}),flush=True)
    ...
    result={'state':'E74_TRAIN_guard_passed' if passed else 'E74_TRAIN_guard_failed','dev_scoring_permitted':bool(passed),
    ...
    write_once(REPORT,result);write_once(EVIDENCE/'e74_fit.json',result)

"""One E66 DEV comparison after E69 TRAIN feasibility; never an independent final."""
...
if __name__=='__main__':
    def denied(*args,**kwargs):raise RuntimeError('E69 DEV network disabled')
...
from experiments import e42_features as dino,e69_model as model
from experiments.e65_acquisition import digest,read,write_once
from experiments.e65_diagnostic import social_q75_bytes
from experiments.e69_fit import validate as validate_fit,CONTRACT as FIT_CONTRACT,CANDIDATE,REPORT as FIT_REPORT
from experiments.e69_features import ROOT,EVIDENCE,resource_check,shuffle_crops
...
def shuffle_view(crops):
    ...
    return list(shuffle_crops(np.repeat(np.asarray(crops)[None],3,axis=0))[0])
...
def freeze():
    ...
    if digest(FIT_REPORT)!=digest(EVIDENCE/'e69_fit.json') or not result['dev_scoring_permitted']:
        raise ValueError('E69 TRAIN guard failed: DEV scoring denied')
    ...
    c={'state':'E69_single_E66_DEV_comparison_registered','inputs':{str(p):digest(p) for p in files},
       ...
       'features':'Unchanged E43 global+2texture crops for each image/transport, plus fixed28px patch shuffle '
                  'on exactly those crops; no crop reselection. Same order/transform as E69 TRAIN.',
    ...
    write_once(CONTRACT,c);write_once(EVIDENCE/'e69_dev_contract.json',c|{'contract_sha256':digest(CONTRACT)})
    return {'state':c['state'],'views':640}


def validate():
    validate_fit();c=read(CONTRACT)
    if digest(CONTRACT)!=read(EVIDENCE/'e69_dev_contract.json')['contract_sha256']:raise ValueError('DEV contract changed')
...
def score():
    ...
    with torch.inference_mode(),threadpool_limits(limits=2):
        for start in range(0,len(rows),c['batch_views']):
            resource_check(deadline);batch=rows[start:start+c['batch_views']];normal=[];shuffled_crops=[]
            for r in batch:
                ...
                normal.extend(pack);shuffled_crops.extend(shuffle_view(pack))
            tensor=torch.from_numpy(np.stack(normal+shuffled_crops)).to(device).permute(0,3,1,2).float().div_(255)
            ...
            features=dino.aggregate_tokens(tokens,2*len(batch));original=features[:len(batch)];shuffled=features[len(batch):]
            old=head.predict_proba(original)[:,1];new=model.predict(head,original,shuffled,arrays)
    ...
    result={'state':'E69_DEV_scores_locked_before_metrics','contract_sha256':digest(CONTRACT),
        'rows':done,'seconds':time.monotonic()-start_time}
    write_once(SCORES,result);write_once(EVIDENCE/'e69_dev_scores.json',{'views':len(done),'scores_sha256':digest(SCORES),
...
def report():
    c=validate()
    if digest(SCORES)!=read(EVIDENCE/'e69_dev_scores.json')['scores_sha256']:raise ValueError('locked DEV scores differ')
    ...
    result={'state':'E69_limited_DEV_comparison_complete','checks':checks,'passes_limited_dev_screen':passed,
    ...
    write_once(REPORT,result);write_once(EVIDENCE/'e69_development.json',result)

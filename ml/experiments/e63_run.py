...
if __name__ == '__main__':
    def denied(*args, **kwargs):
        raise RuntimeError('E63 network connections disabled')
...
from experiments import e63_constrained as model
...
def resource_check(deadline):
    if time.monotonic() > deadline:
        raise TimeoutError('E63 runtime budget exceeded')
...
def freeze():
    ...
    for key, path in {'model_code': Path(model.__file__), 'runner_code': Path(__file__),
                      ...
                      'helper_code': Path(__file__).with_name('e61_replay_gate.py'),
                      'constraint_code': Path(__file__).with_name('e62_constrained.py')}.items():
        inputs[key] = {'path': str(path), 'sha256': digest(path)}
    for item in inputs.values():
        if digest(item['path']) != item['sha256']:
            raise ValueError('admitted input changed')
    config = {'state': 'E63_constrained_TRAIN_fit_registered', 'inputs': inputs,
              ...
              'correction': 'Gaussian RBF64 from weighted KMeans64 on E43-false-positive TRAIN REAL; seed63 n_init3 max_iter100. '
                            'Sigma=median positive assigned-center distance. Center/standardize RBF on all TRAIN; '
                            'append intercept, additive linear logit coefficients, zero initialization.',
              ...
              'constraints': 'All reference-caught TRAIN AI stay at/above the fixed AI logit cut with '
                             'min(existing_margin,1e-7) retained. Correct TRAIN REAL correction <=0.',
              ...
              'fit_guard': 'Solver success, violation<=1e-8, exact saved replay; E61 zero new AI misses; '
                           'zero new REAL false alarms; REAL FPR strictly lower in all3 TRAIN conditions.',
    ...
    write_once(EVIDENCE / 'e63_contract.json', config | {'contract_sha256': digest(CONTRACT)})
    return {'state': config['state'], 'contract_sha256': digest(CONTRACT)}


def validate():
    config = read(CONTRACT)
    if digest(CONTRACT) != read(EVIDENCE / 'e63_contract.json')['contract_sha256']:
...
def fit():
    ...
    if RESULT.exists() or CANDIDATE.exists():
        raise FileExistsError('E63 already executed; no refit or replacement')
    ...
    with threadpool_limits(limits=2):
        ...
        z = model.pca_project(head, features, arrays['mean'], arrays['components'], arrays['scales'])[:, :-1]
        hard = (labels == 0) & (baseline >= model.AI_CUT)
        center_weights = balanced_parent_weights(labels, sources, parents)[hard]
        clusters = KMeans(n_clusters=64, random_state=63, n_init=3, max_iter=100)
        clusters.fit(z[hard], sample_weight=center_weights)
        distances = np.linalg.norm(z[hard] - clusters.cluster_centers_[clusters.labels_], axis=1)
        sigma = float(np.median(distances[distances > 0]))
        kernel = model.kernels(z, clusters.cluster_centers_, sigma)
        arrays.update(centers=clusters.cluster_centers_, sigma=np.asarray(sigma),
                      kernel_mean=kernel.mean(axis=0), kernel_scales=kernel.std(axis=0))
        if not np.isfinite(arrays['kernel_scales']).all() or np.any(arrays['kernel_scales'] <= 0):
            raise ValueError('degenerate TRAIN kernel feature')
        del kernel, z
        x = model.project(head, features, arrays)
    ...
    accepted = (solver['success'] and solver['max_constraint_violation'] <= 1e-8
                ...
                and all(r['new_ai_rate'] < r['old_ai_rate'] for r in real))
    resource_check(deadline)
    if digest(paths['reference']) != REFERENCE_SHA:
        raise ValueError('reference changed')
    result = {'state': 'E63_TRAIN_feasible' if accepted else 'E63_TRAIN_guard_failed',
    ...
    write_once(RESULT, result); write_once(EVIDENCE / 'e63_fit.json', result)

"""A single E92-preserving correction using all additional admitted MIDD TRAIN views."""
import argparse
import fcntl
import json
import os
...
if __name__ == '__main__':
    def denied(*args, **kwargs):
        raise RuntimeError('E102 fit is offline')
...
from experiments.e81_real_minimax import fit as fit_delta
from experiments.e91_data import load as load_old, CONDITIONS
from experiments.e92_fit import validate as validate_base
...
ROOT = DATA_ROOT/'e102'; EVIDENCE = ML_ROOT.parent/'evidence'
CONTRACT = ROOT/'fit_contract.json'; CANDIDATE = ROOT/'correction.npz'; REPORT = ROOT/'fit.json'
BASE = DATA_ROOT/'e92/correction.npz'
...
def freeze():
    ...
    old_report = read(DATA_ROOT/'e92/fit.json')
    if digest(DATA_ROOT/'e101/features.json') != read(EVIDENCE/'e101_features.json')['report_sha256'] or \
            ...
            digest(DATA_ROOT/'e92/fit.json') != digest(EVIDENCE/'e92_fit.json') or \
            digest(BASE) != old_report['candidate_sha256'] or not old_report['dev_scoring_permitted']:
        raise ValueError('Complete E101 cache and frozen E92 required')
    paths = [Path(__file__), Path(__file__).with_name('e101_features.py'), Path(__file__).with_name('e91_data.py'),
        ...
        Path(__file__).with_name('e81_real_minimax.py'), Path(__file__).with_name('e73_fit.py'),
        ...
        DATA_ROOT/'e101/features_contract.json', DATA_ROOT/'e101/features.json', DATA_ROOT/'e92/fit.json']
    c = {'state': 'E102_frozen_map_MIDD_correction_registered', 'inputs': {str(p): digest(p) for p in paths},
        ...
        'objective': 'Frozen E92 map,450 zero-initialized delta weights, E81 worst-REAL epigraph '
            'loss, all E92 AI logits nondecreasing, correct REAL binary decisions protected; no sweep.',
        ...
        'limits': 'TRAIN-only data-coverage fit; no calibration or independent-source validation. '
    ...
    write_once(EVIDENCE/'e102_fit_contract.json', c | {'contract_sha256': digest(CONTRACT)})
    return {'state': c['state'], 'views': c['views']}


def validate():
    validate_base(); validate_features(); c = read(CONTRACT)
    if digest(CONTRACT) != read(EVIDENCE/'e102_fit_contract.json')['contract_sha256']:
...
def fit():
    ...
    with np.load(BASE, allow_pickle=False) as a:
        arrays = {k: np.array(a[k], copy=True) for k in a.files if k not in ['contract_sha256', 'reference_sha256']}
    ...
    with threadpool_limits(limits=2):
        ...
        if not np.array_equal(baseline, model.predict(head, features['dino'], features['clip'], features['dear'], arrays)):
            raise ValueError('Zero-delta E92 replay differs')
        if old_numeric(old, baseline[:len(old)*4].reshape(-1, 4), 11630, 12141) != read(DATA_ROOT/'e92/fit.json')['full_TRAIN_metric_gates']:
            raise ValueError('Frozen E92 TRAIN metrics differ')
        initial_new = new_sensor_gates(new, baseline[len(old)*4:].reshape(-1, 4))
        calls = [0]
        def check():
            resource_check(deadline); calls[0] += 1
            if calls[0] % 10 == 0:
                print(json.dumps({'E102_iteration': calls[0], 'seconds': round(time.monotonic()-start)}), flush=True)
    ...
    result = {'state': 'E102_TRAIN_guard_passed' if passed else 'E102_TRAIN_guard_failed',
        ...
        'expanded_numeric_gates': expanded, 'baseline_new_sensor_gates': initial_new, 'new_sensor_gates': new_gates,
        'runtime_batch_replay': runtime, 'seconds': time.monotonic()-start, 'dev_final_rows_read': 0, 'promotion_allowed': False}
    write_once(REPORT, result); write_once(EVIDENCE/'e102_fit.json', result)

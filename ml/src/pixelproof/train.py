...
def main() -> None:
    ...
    train_loader, val_loader = build_loaders(Path(config["data"]["root"]), config["data"]["image_size"], config["training"]["batch_size"], config["data"]["validation_ratio"], config["seed"])
    ...
    for epoch in range(1, config["training"]["epochs"] + 1):
        ...
        if val_accuracy > best_accuracy:
            ...
            torch.save({"model": model.state_dict(), "config": config, "val_accuracy": val_accuracy}, output / "best.pt")

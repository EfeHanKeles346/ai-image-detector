...
def journal(text):
    ...
    for path in [REPO/'HISTORY.md', REPO/'ml/EXPERIMENTS.md']:
        with path.open('a') as f:
            ...
            f.write(f'\n### E124/E125 unattended execution — {stamp}\n\n{text}\n')
...
def main(api_pid):
    ...
    with (RUN/'execution.lock').open('a') as lock:
        ...
        failed = None
        try:
            journal('Started the fixed full E124 extraction; temporarily stopped the verified E92 API for GPU memory. Durable chunks and stage logs are local. This is feature extraction, not a completed model improvement.')
            run_stage('experiments.e124_fullframe_features', 'extract')
            result = json.loads((ROOT/'e124/report.json').read_text())
            journal(f"E124 completed {result['parents']} parents / {result['views']} TRAIN views; maximum full-frame pilot replay error {result['max_pilot_error']}. Registering the prespecified paired E125 fit; no DEV/final opened.")
            run_stage('experiments.e125_fullframe_fit', 'freeze')
            run_stage('experiments.e125_fullframe_fit', 'fit')
            result = json.loads((ROOT/'e125/report.json').read_text())
            journal('E125 paired TRAIN fits completed. '+json.dumps(result['summary'], sort_keys=True)+
                ' These are TRAIN guards only; serving/promotion and independent evaluation remain unchanged. Full reports are in evidence/e125_fullframe_fit.json.')
